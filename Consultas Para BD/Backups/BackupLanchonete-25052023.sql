...
-- Copiando dados para a tabela lanchonete3si_2023.auditoria: ~7 rows (aproximadamente)
...
INSERT INTO `auditoria` (`codAuditoria`, `acao`, `tabela`, `dataHora`, `usuario`) VALUES
	...
	(7, 'Produto excluído = 5', 'produto', '2023-05-23 10:31:02', 'root@localhost'),
	(8, 'Produto excluído = 5', 'produto', '2023-05-23 10:31:07', 'root@localhost'),
	(9, 'Produto excluído = 4', 'produto', '2023-05-23 10:31:26', 'root@localhost');
...
BEGIN
	...
	SELECT COUNT(*) INTO @contador FROM produto WHERE codProduto = codApagar;
	
	if(@contador = 0)
		then SELECT "Produto não encontrado" AS erro;
			ELSE DELETE FROM produto WHERE codProduto = codApagar;
	END if;
...
-- Copiando estrutura para procedure lanchonete3si_2023.proc_relatorioVendasClientes
...
BEGIN
	...
	SELECT COUNT(*) INTO @contador FROM venda AS v
	WHERE v.CLIENTE_codCliente = codClientePesquisar;
	
	if(@contador = 0)
		then SELECT "Cliente não possui compras" AS mensagem;
			ELSE SELECT v.codVenda, date_format(v.dataHora,"%d de %M de %Y %H:%i:%s") AS dataHora, 
			c.nome
...
-- Copiando dados para a tabela lanchonete3si_2023.venda: ~3 rows (aproximadamente)
